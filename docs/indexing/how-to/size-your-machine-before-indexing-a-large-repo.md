...
`indexing/reference/storage-schema-reference`). There's no separate budget
...
`indexing/how-to/speed-up-or-debug-a-slow-index-run` for what to check if
a specific run is slower than file count alone would explain, and
`indexing/explanation/how-indexing-works` for what each pipeline stage
