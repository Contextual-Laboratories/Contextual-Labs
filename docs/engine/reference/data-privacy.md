...
`engine/explanation/how-indexing-works` for the full pipeline.
...
See `engine/explanation/embedding-model-stack`.
...
- **Symlinks are never followed** during file discovery — a symlink
  ...
  `engine/reference/contextualignore-reference`.
...
`engine/reference/platform-support` for the platform-specific detail).
