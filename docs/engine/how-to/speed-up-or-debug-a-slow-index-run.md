...
`engine/reference/contextualignore-reference` for syntax — and confirm
...
-index rebuild (see `engine/explanation/how-indexing-works` for what each
...
- Slow at **graph extraction**: proportional to file count and language
  ...
  `engine/reference/language-support-matrix`) cost more here than
  chunk-only languages.
- Slow at **chunk + embed**: this is the CPU-bound stage — see
  `engine/how-to/size-your-machine-before-indexing-a-large-repo` for what
...
`engine/explanation/how-indexing-works` first; the actual order is
...
`engine/observability/interpreting-doctor-report` — to rule out an
