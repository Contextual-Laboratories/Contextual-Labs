...
`engine/explanation/temporal-intelligence`). Two are worth calling out
...
`graph_find_path`, and `graph_query` — see `mcp-server/reference/` for
each tool's exact parameters. `engine/explanation/nexus-enrichment`
