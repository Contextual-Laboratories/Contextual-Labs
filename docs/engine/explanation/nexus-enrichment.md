...
related:
  ...
  - mcp-server/reference/nexus_search.md
---
...
3. From those seeds, a structural breadth-first traversal walks outward
   ...
   the rest of the taxonomy in `engine/explanation/the-knowledge-graph`).
4. Every node picked up along the way gets its temporal context attached
   ...
   (see `engine/explanation/temporal-intelligence`).
...
`engine/explanation/how-indexing-works` has run), `nexus_search` falls
...
one function). See `mcp-server/how-to/choose-between-search-and-nexus-search`
