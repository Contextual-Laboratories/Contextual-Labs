...
`engine/reference/contextualignore-reference`). Runs in a thread pool so it
...
`engine/explanation/the-knowledge-graph` for what actually gets extracted.
...
model runs locally, CPU-only. See `engine/reference/data-privacy` for the
...
`engine/explanation/incremental-vs-scheduled-indexing` for how that path
differs.
