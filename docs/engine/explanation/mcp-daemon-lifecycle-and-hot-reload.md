...
related:
  ...
  - engine/how-to/restart-the-daemon-after-a-code-or-config-change.md
...
see `engine/explanation/mcp-client-access-control`) without a full
...
`engine/explanation/embedding-model-stack` for more on this.
...
`engine/observability/interpreting-doctor-report`. If the daemon seems up
but isn't responding to tool calls, see
`engine/troubleshooting/daemon-not-responding`.
