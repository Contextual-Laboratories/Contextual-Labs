...
related:
  ...
  - observability/how-to/interpreting-doctor-report.md
...
| Indexing itself seems slow or stuck | See `indexing/how-to/speed-up-or-debug-a-slow-index-run` first — its stage-name output usually tells you more directly than either log file |
| You don't know if the daemon is even healthy | `contextual doctor` — see `observability/how-to/interpreting-doctor-report` |

## File locations and rotation, at a glance

Both files rotate rather than growing unbounded — see
`observability/reference/logs-and-retention-reference` for the exact rotation
